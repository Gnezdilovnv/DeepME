...
class ChatFragment : Fragment() {
    ...
    private lateinit var inf: TextView; private val msgs = ArrayList<CM>(); private var cp = "основной"; private val prs = ArrayList<String>()
    ...
    override fun onViewCreated(v: View, b: Bundle?) {
        ...
        msp.adapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, mn).also { it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }
        ...
        msp.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(p: AdapterView<*>?, v: View?, pos: Int, id: Long) {
                ...
                inf.text = "💰 Вход: ${ApiClient.models[pos].priceIn} | Выход: ${ApiClient.models[pos].priceOut} за 1M"
            }
            override fun onNothingSelected(p: AdapterView<*>?) {}
        }
        val dm = ApiClient.models.first { it.id == ApiClient.deepSeekModel }
        inf.text = "💰 Вход: ${dm.priceIn} | Выход: ${dm.priceOut} за 1M"

        prs.add("основной"); loadPrj(); updPrj()
        npb.setOnClickListener { newProject() }
        clb.setOnClickListener { clearChat() }

        if (TokenManager.isAuthorized(requireContext())) addMsg("🤖 DeepME v2.0 готов\nМодель: ${dm.name}", false)
        else addMsg("⚠️ Настройте API ключи в Настройках", false)

        snd.setOnClickListener { val t = inp.text.toString().trim(); if (t.isNotEmpty()) send(t) }
    }

    private fun showMenu(m: CM) = AlertDialog.Builder(requireContext()).setItems(arrayOf("Копировать", "Поделиться")) { _, w ->
        when (w) {
            0 -> { (requireContext().getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(ClipData.newPlainText("", m.text)); Toast.makeText(context, "✅", Toast.LENGTH_SHORT).show() }
            1 -> startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply { type = "text/plain"; putExtra(Intent.EXTRA_TEXT, m.text) }, ""))
        }
    }.show()
    ...
    private fun newProject() {
        ...
        AlertDialog.Builder(requireContext()).setTitle("Новый проект").setView(et)
            .setPositiveButton("Создать") { _, _ ->
                ...
                if (n.isNotEmpty() && !prs.contains(n)) { saveHist(); prs.add(n); savePrj(); updPrj(); psp.setSelection(prs.indexOf(n)); msgs.clear(); cp = n; ad.notifyDataSetChanged(); addMsg("📂 $n", false) }
            }.setNegativeButton("Отмена", null).show()
    }

    private fun clearChat() = AlertDialog.Builder(requireContext()).setTitle("Очистить?").setPositiveButton("Да") { _, _ -> msgs.clear(); ad.notifyDataSetChanged(); saveHist(); addMsg("🗑️", false) }.setNegativeButton("Нет", null).show()

    private fun send(t: String) {
        if (!TokenManager.isAuthorized(requireContext())) { addMsg("❌ Нет ключей", false); return }
        inp.setText(""); addMsg(t, true); saveHist(); prg.visibility = View.VISIBLE; snd.isEnabled = false
        lifecycleScope.launch {
            ...
            } catch (e: Exception) { prg.visibility = View.GONE; snd.isEnabled = true; addMsg("❌ ${e.message}", false) }
        }
    }
    ...
    inner class ChatAdapter(private val ms: List<CM>, private val olc: (CM) -> Unit) : RecyclerView.Adapter<ChatAdapter.VH>() {
        ...
        override fun onBindViewHolder(h: VH, pos: Int) {
            val m = ms[pos]; h.tv.text = m.text; h.tv.setTextIsSelectable(true); h.tv.movementMethod = LinkMovementMethod.getInstance()
            h.tv.setBackgroundResource(if (m.isUser) R.drawable.bg_user_message else R.drawable.bg_assistant_message)
            h.tv.setTextColor(if (m.isUser) 0xFFFFFFFF.toInt() else 0xFFE6EDF3.toInt()); h.itemView.setOnLongClickListener { olc(m); true }
        }
