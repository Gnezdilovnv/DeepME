...
class SettingsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val modelNames = ApiClient.models.map { "${it.name} — ${it.desc}" }
        modelSpinner.adapter = ArrayAdapter(requireContext(), android.R.layout.simple_spinner_item, modelNames)
        ...
        if (TokenManager.isAuthorized(requireContext())) {
            ...
            statusText.setTextColor(0xFF238636.toInt())
            checkBalance()
        }

        saveBtn.setOnClickListener {
            val ds = dsInput.text.toString().trim()
            val gh = ghInput.text.toString().trim()
        ...
        lifecycleScope.launch {
            try {
                ...
                val response = withContext(Dispatchers.IO) {
                    ApiClient.deepSeekApi.chat(
                        "Bearer $key",
                        DeepSeekRequest(model = ApiClient.deepSeekModel, messages = listOf(Message("user", "ping")), max_tokens = 1)
                    )
                }
                val model = ApiClient.models.first { it.id == ApiClient.deepSeekModel }
                balanceText.text = "💰 Баланс активен | ${model.name}\nВход: ${model.priceIn}$ | Выход: ${model.priceOut}$ за 1M токенов"
                balanceText.setTextColor(0xFF238636.toInt())
            } catch (e: Exception) {
                balanceText.text = "💰 Ошибка: ${e.message?.take(80)}"
                balanceText.setTextColor(0xFFf85149.toInt())
            }
        }
    }
}